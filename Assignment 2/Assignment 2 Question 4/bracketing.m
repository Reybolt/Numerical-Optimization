...
function [retcode, ax, af, ag, alpha] = bracketing(x, f, g, d, alphamax, sigma, rho, tau1, tau2,tau3, lowerbd);
...
mu = min(alphamax, (lowerbd - f_zero)/(rho*df_zero));
alpha = mu/2;  
...
while k < 20;
  ...
  if f_alpha > f_zero + alpha*rho*df_zero | f_alpha >= f_alpha_prev;
    ...
    [retcode, ax, af, ag, alpha] = sectioning(x, ax, g, d, alpha, rho, tau2, tau3, sigma, a, b, f_zero, df_zero, f_a, f_b, df_a, 0, alphamax);
    return;
  end;
